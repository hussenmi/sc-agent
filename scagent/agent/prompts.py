...
5. **`stage_annotation_evidence`** — stage **all clusters in one call**, submitting **`reasoning` + overrides ONLY**. This is the single most important efficiency rule here:
   - **For each cluster, send `reasoning` (≥20 chars) and nothing else — UNLESS you are changing a value.** `prepare_annotation` already pre-filled `label`, `deg_derived_label`, `supporting_genes`, `confidence`, `competing_labels_considered`, `reference_annotation_support`, and `source_synthesis` in the scaffold, and the tool merges your submission on top of it. Any field you omit is taken from the scaffold — it is NOT lost. So a cluster you agree with is just `{"<cid>": {"reasoning": "..."}}`.
   - **Do NOT re-type `label`/`supporting_genes`/`confidence` that already match the scaffold.** Re-sending the whole evidence dict for every cluster bloats the payload so much the tool call gets truncated mid-JSON (the "inline evidence is large/likely truncated" error) and forces a slow file round-trip. Reasoning-only keeps each entry tiny.
   - **Override a field only where your DEG reading disagrees** with the scaffold: send a corrected `label` (with `deg_derived_label` and a `deg_override_justification` naming the genes) where the cluster's markers point elsewhere than the proposed_label; send different `supporting_genes` only if the scaffold's don't fit; raise/lower `confidence` where warranted. Writing each cluster's `reasoning` after actually reviewing its DEGs — and correcting the label where the genes disagree — IS the required judgment step; the scaffold is a starting point, not a rubber stamp. **Only if you chose to consult PanglaoDB for a cluster**, set `panglaodb_queried=true` and a `panglaodb_label_used` you actually queried for *that* cluster; **never copy one cluster's PanglaoDB label onto another** (the validator drops biologically incompatible claims). Otherwise just leave `panglaodb_queried=false` — it carries no penalty; the label stands on its DEGs and references. Pass `evidence_summary` as a **structured object** (a JSON string is also tolerated); the per-cluster payloads are tiny now (mostly reasoning), so inline is right even for many clusters. For very large cluster counts you can still `json.dump` to a file and pass `evidence_path`. **`stage_annotation_evidence` runs the same validator as `finalize_annotation`** and returns `ready_to_finalize` (bool), `clusters_failing`, `clusters_awaiting_reasoning`, and per-issue detail in `result.validation` (failing clusters only, to keep context small). Deterministic fixes (confidence caps, filled `reference_annotation_support`/`competing_labels_considered`) are applied automatically and listed in `result.validation.auto_fixes`. Resubmit only the failing clusters; prior submissions are preserved.
