...
**Cluster structure QC ALWAYS runs and runs together with metric QC.** `run_cluster_qc` **auto-runs structure QC in the same call** — gene-gene covariance modules, clustered correlation heatmaps, and technical Moran's I — and returns one combined cleanup recommendation (`structure_qc.synthesized_removal`). Crucially it runs **even when nothing is metric-flagged**: "metric-clean" is NOT "coherent" — metric QC cannot see a doublet/noise mixture with normal library size, genes, and MT%, and if Scrublet was skipped there is no doublet signal at all (the case where a coherence check matters MOST). So on a clean pass it runs a **baseline coherence check over all clusters**. Coherence metrics are computed for every cluster; heatmap figures are capped (`max_heatmaps`, default 20) — plotted for the non-coherent clusters worth eyeballing, while coherent clusters get a recorded verdict without a figure. You normally do NOT call `run_cluster_structure_qc` separately. **Narrate what it found, not just that it ran**: report the `structure_qc.structure_summary` / `coherence_breakdown` — how many clusters were coherent vs unstructured/weak (possible mixtures) vs inconclusive, how many heatmaps were saved, and the synthesized removal set — and cite specific heatmap paths for any cluster you call out. Enforced two ways: (1) `prepare_annotation` **refuses** until structure QC has run on the active clustering; and (2) the run cannot end if metric QC ran but structure QC never did. It re-runs per clustering. The ONLY exception is an explicit user opt-out (`allow_skip_structure_qc=true`).
