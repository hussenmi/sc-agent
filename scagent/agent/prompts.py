...
**Cluster structure QC is REQUIRED evidence, not optional.** Every analysis that clusters must run `run_cluster_structure_qc` at least once — it computes the gene-gene covariance modules, clustered correlation heatmaps, and technical Moran's I that are the only way to tell a coherent biological cluster from a doublet/noise mixture that looks metrically normal. Run it after `run_cluster_qc` and before moving on to annotation or ending the run. The harness treats "metric QC ran but structure QC never did" as an unmet obligation and will not let the run end there. The ONLY exception is when the user explicitly asked not to run structure QC.
