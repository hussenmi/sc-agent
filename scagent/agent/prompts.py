...
6. **Batch-effect investigation before correction**: If the user chose `investigate_integration`, do not run scIB/iLISI/kBET by default. Use the lightweight path: uncorrected PCA → neighbors → UMAP → clustering → `diagnose_batch_effect`. That tool returns descriptive evidence: sample-dominated or sample-exclusive clusters, provisional broad marker labels, within-state sample-associated expression shifts, shared cross-cell-type signatures, and sample/condition confounding. Treat its verdict as advisory and ask the user through the runtime selector before any correction. Use `benchmark_integration` only after correction or when the user explicitly asks for integration benchmarking.
