...
5. **`stage_annotation_evidence`** — stage **all clusters in one call** whenever you can (batching is only for very large cluster counts). Each entry is keyed by cluster_id and should include `{label, deg_derived_label, supporting_genes, panglaodb_queried, panglaodb_label_used, competing_labels_considered, confidence, reasoning, source_synthesis}` plus `reference_annotation_support` and `reference_annotation_conflicts` when reference labels were used, and `deg_override_justification` whenever `deg_derived_label` differs from the final `label`. **`deg_derived_label`** is the label the cluster's own top DEGs indicate, derived before/independently of the reference tools — it is required for every cluster (markers have the final say). **For clusters whose `validation_tier` is `reference_consensus_plus_deg` or `reference_partial_plus_deg`, set `panglaodb_queried=false` and OMIT `panglaodb_label_used` entirely — do not invent one.** Only `needs_external_adjudication` clusters carry `panglaodb_queried=true` with a `panglaodb_label_used` you actually queried for *that* cluster. **Never copy one cluster's PanglaoDB label onto another** (e.g. don't put `panglaodb_label_used='dendritic cells'` on a T-cell cluster just to make `panglaodb_queried` true) — the validator drops biologically incompatible claims, and a low/medium confidence label needs no PanglaoDB if reference + DEGs already support it. If you could not find a PanglaoDB label that genuinely matches such a cluster, do NOT force a near-miss: set `panglaodb_queried=false` and `confidence='low'` — the validator accepts the cluster on reference + DEG evidence and caps it to low automatically. `source_synthesis` should be an object with at least `agreement` and `final_decision_basis`, not a free-text string. Pass `evidence_summary` as a **structured object argument** (real JSON; a JSON string is also tolerated and parsed) — for up to ~25 clusters this inline object is the right call and needs no file. For larger sets, **build the dict in `run_code` and `json.dump` it to a file** (`(Path(output_dir)/'annotation_evidence.json').write_text(json.dumps(evidence))`), then pass that path as `evidence_path`. Do **not** route annotation evidence through `write_json` — stringifying a 20+-cluster payload tends to truncate and fail; the inline object and the `run_code`+`json.dump`+`evidence_path` paths are both reliable. When building the dict in `run_code`, construct it programmatically (normal `{...}` literals) — never paste a giant pre-serialized JSON string into the code. **`stage_annotation_evidence` runs the same validator as `finalize_annotation`** and returns `ready_to_finalize` (bool), `clusters_failing` (list), and per-issue detail in `result.validation` — read that report. Deterministic fixes (e.g., lowering confidence when only a broader lineage was validated, or honouring the QC-derived cap) are applied automatically and listed in `result.validation.auto_fixes`. Resubmit `stage_annotation_evidence` with only the failing clusters corrected; staged evidence is preserved across calls, so you do not need to re-send what already validated.
6. **Finalize** — call `finalize_annotation` **only when `stage_annotation_evidence` returned `ready_to_finalize: true`** (`clusters_failing` empty and all clusters covered). Do not call finalize speculatively while clusters are still failing — that just wastes a round-trip, since finalize runs the identical validator. Once ready, finalize commits the labels (evidence_summary can be omitted; staged evidence is reused). Because staging already validated, `finalize_annotation` should normally pass on the first call. If you skip staging and call `finalize_annotation` directly, the same rules apply: it refuses to write labels if `prepare_annotation` was not run, if any cluster lacks evidence (unless `allow_partial=true`), if any cluster lacks `supporting_genes`, reference-support notes when reference columns were used, explicit reasoning, source synthesis, required PanglaoDB evidence, or competing-label records for ambiguous clusters. Missing CellTypist sources must either be run or have concrete unavailable reasons recorded. Missing Scimilarity is stricter: when the package and model path are available, `finalize_annotation` requires `run_scimilarity`; a manually supplied `reference_source_unavailable` value is not enough. Nuisance genes (MT/ribosomal/hemoglobin/MALAT1/generic locus genes) cannot be the sole support. Broad-parent PanglaoDB labels, weak one-source support, and QC caveats auto-cap confidence. On success it writes `adata.obs[annotation_key]` and records full evidence in `adata.uns['annotation_validation']`. When a cluster's `supporting_genes` are rejected as not in its DEGs, the failure message now lists that cluster's pre-validated `suggested_supporting_genes` — cite those directly to fix it in one round rather than guessing. As a last resort, if finalize genuinely cannot pass after honest correction rounds, call `save_data` with `allow_unvalidated=true`: it writes a clearly-marked `_UNVALIDATED` dataset (with `adata.uns['annotation_status']='unvalidated'`) so the analysis is never lost — never abandon the run with nothing saved, and never hand-assign labels in `run_code` to dodge validation.
