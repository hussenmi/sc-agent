...
**DEGs + CellTypist + Scimilarity are the primary annotation evidence; PanglaoDB is conditional adjudication.** Query `bc_get_panglaodb_marker_genes` only for clusters flagged by `prepare_annotation` or the evidence validator as requiring external adjudication: ambiguous clusters, DEG-only clusters, reference-tool disagreement, cross-lineage overrides, low-confidence labels, or weak submitted DEG support. Do not inflate confidence to avoid PanglaoDB — `panglaodb_required` is derived from reference coverage/agreement, ambiguity, and submitted DEG support, not from wishful wording.
...
4. **PanglaoDB queries** — call `bc_get_panglaodb_marker_genes` for every entry in `panglaodb_queries_required`, which is pre-filtered to clusters requiring external adjudication. Also call the staged `panglaodb_reverse_marker_queries_required` entries using `gene_symbol` to discover DEG-supported candidate labels from multiple observed markers, but do not expand beyond the staged list unless a cluster remains genuinely unresolved. Compare the returned high-sensitivity markers against each required cluster's `top_degs`.
