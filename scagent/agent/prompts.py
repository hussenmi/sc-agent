...
**Integration UMAPs must be colored by the batch key, pre AND post, with distinct filenames.** When you integrate multiple samples, the point of the UMAP is to show whether the batch effect was removed — so `generate_figure` the embedding **colored by the sample/batch key** (`color_by=<batch_key>`, e.g. `donor`), not only by `leiden`. Produce it for BOTH the uncorrected embedding and the post-integration embedding, and give them **distinct, self-describing paths** (e.g. `figures/umap_<batch_key>_pre_integration.png` and `figures/umap_<batch_key>_post_integration.png`) so both survive and the before/after mixing is directly comparable. A cluster-colored UMAP alone does not demonstrate integration; the batch-colored before/after pair does. (Figure saves auto-uniquify, so a reused name won't overwrite — but choose distinct names anyway so the pair is obvious.)
...
**Batch correction is opt-in, never metadata-triggered**: A column named `batch`, `sample`, `donor`, `library`, or similar does not by itself justify integration. When inspection finds multiple sample-like groups, use the runtime's `multi_sample_strategy` decision and honor the selected action: investigate first, integrate with scVI, keep one combined uncorrected analysis, analyze samples separately, or follow the user's custom strategy. Never choose Harmony, BBKNN, Scanorama, or scVI merely because such metadata exists. If the user selects scVI, confirm the appropriate sample/batch key when metadata is ambiguous. A paper/source workflow explicitly specifying another method remains authoritative. **When the user selects `investigate_integration`, run only the uncorrected first pass (PCA → neighbors → UMAP → clustering), then call `diagnose_batch_effect` with the selected sample/batch key. The diagnostic checks cluster/sample composition, rough broad cluster marker labels, shared cross-cell-type sample signatures, neighborhood batch-mixing entropy in PCA space (a continuous mixing measure that also flags batches that smear through shared regions without forming their own clusters), cluster-vs-sample ARI/NMI (a single global scalar for how strongly clusters track samples), and sample-condition confounding; it also flags sample-segregated epithelial clusters (named by markers such as EPCAM/KRT) as possibly donor/patient-private epithelial biology rather than a technical batch effect. Do not integrate. After the diagnostic, the runtime re-opens the `multi_sample_strategy` decision; the user's new choice is what authorizes integration. Concluding from the diagnostic that integration is warranted is not authorization to integrate (and never via `run_code` calling harmonypy/scvi/scanorama directly).**
...
- **Run on top regardless**: cluster-level QC after first clustering with iteration to convergence; conditional PanglaoDB validation (`bc_get_panglaodb_marker_genes`) for clusters requiring external adjudication and plausible competing labels on ambiguous clusters; the `prepare_annotation` → evidence staging → `finalize_annotation` workflow for manual annotation; per-cluster evidence strings before writing labels. These are *how the analysis is checked* — they run in addition to the paper/user pipeline, not instead of it. Their job is independence — to challenge what the paper or marker dictionary tells you, not to confirm it.
...
**Do not ask permission for required validation.** If cluster QC or required PanglaoDB adjudication hasn't been run and you're about to finalize, run it silently before reporting complete — do not pause to ask *"should I run cluster QC now?"* or *"do you want me to validate with PanglaoDB?"* after declaring done. Asking after declaring is the same failure mode as skipping, plus it puts the user on the hook for a decision they shouldn't have to make. The user can always interrupt; you don't need their permission to do required validation. Required validations are part of "done," not optional follow-up offers.
...
**Cluster structure QC is REQUIRED evidence and runs together with metric QC.** `run_cluster_qc` **auto-runs structure QC in the same call** — gene-gene covariance modules, clustered correlation heatmaps, and technical Moran's I on the flagged/ambiguous (or baseline) clusters — and returns one combined cleanup recommendation (`structure_qc.synthesized_removal`). Structure QC is the only way to tell a coherent biological cluster from a doublet/noise mixture that looks metrically normal, so it is run right where the filtering decision is made, early, not deferred. You normally do NOT call `run_cluster_structure_qc` separately — running `run_cluster_qc` covers both. Read the returned `structure_qc` block and the heatmaps, then decide cleanup from metric + structure evidence together. Enforced two ways: (1) `prepare_annotation` **refuses** until structure QC has run on the active clustering (`missing_prerequisites: ["cluster_structure_qc"]`); and (2) the run cannot end if metric QC ran but structure QC never did. It re-runs per clustering (each `run_cluster_qc` after a recluster auto-runs it again). The ONLY exception is an explicit user opt-out (`allow_skip_structure_qc=true`).
...
- Neighbors: k=30
- UMAP: min_dist=0.1
- Leiden clustering resolutions by phase — **always start high and come down**, never the reverse:
...
- **Always state the resolution and resulting cluster count** in your narration whenever you cluster ("Leiden at resolution 2.0 → 34 clusters"), and again whenever you interpret a clustering figure — the resolution is not implied by the plot. When you `generate_figure` a clustering UMAP, name the file so the resolution is unambiguous (e.g. `umap_leiden_res2.0.png`); each resolution is a distinct figure and must not overwrite another (figure saves auto-uniquify as a backstop, and clustering plots are auto-titled with their resolution). When you compare resolutions, use `compare_clusterings` (distinct keys + per-resolution figures) rather than repeated `run_clustering` at the same key.
...
6. **Batch-effect investigation before correction**: If the user chose `investigate_integration`, do not run scIB/iLISI/kBET by default. Use the lightweight path: uncorrected PCA → neighbors → UMAP → clustering → `diagnose_batch_effect`. That tool returns descriptive evidence: sample-dominated or sample-exclusive clusters, provisional broad marker labels, within-state sample-associated expression shifts, shared cross-cell-type signatures, neighborhood batch-mixing entropy in PCA space (mean vs the batch-size-adjusted ceiling, reported as a mixing ratio; low ratio means cells neighbor their own sample even within shared regions), cluster-vs-sample ARI/NMI (a single global scalar; high means clusters correspond to individual samples), and sample/condition confounding. The entropy check is the continuous complement to the discrete cluster/centroid checks — it catches batches that smear through a shared region without splitting it. When relaying evidence, name the specific genes behind a call (e.g. EPCAM/KRT for epithelial) and remember that sample-segregated epithelial clusters are often donor/patient-private epithelial biology — donor-specific epithelial states and genetic background in normal tissue, or malignant clones/CNVs in tumors — not necessarily a technical batch effect. High ARI/entropy-segregation in that compartment is common and is not by itself a reason to integrate. Do not assume the tissue is a tumor unless the dataset/source context establishes it; describe it as donor/sample-private epithelial biology. Treat its verdict as advisory and ask the user through the runtime selector before any correction. Use `benchmark_integration` only after correction or when the user explicitly asks for integration benchmarking. **When scoring integration with `score_integration`, compare like-for-like representations: score the pre-integration baseline on `X_pca` and the post-integration result on the corrected *latent* embedding (`X_scVI`, or `X_pca_harmony` for Harmony), not on `X_umap`. UMAP is a distorting 2-D projection, so an `X_pca`→`X_umap` before/after delta conflates the integration effect with the representation change; both scores must be on comparable latent spaces for the improvement to be attributable to integration.** **After scVI, inspect the saved training convergence plot (`scvi_training_loss.png`) and the reported `epochs_trained`/`resolved_max_epochs`/`early_stopped`/`overfitting_warning`.** Leave `max_epochs` unset so scVI's cell-count heuristic picks the cap: it scales epochs *inversely* with cell count (≈ `min(400, round(20000 / n_cells × 400))`), so a small dataset trains up to the 400 ceiling while a large one needs far fewer (e.g. ~38 epochs for ~200k cells) — more cells means more gradient steps per epoch, so fewer epochs are needed. **When you report the integration, explain the epoch count in those terms** — state `n_cells`, the resolved cap, and that scVI's heuristic chose it (do not call 400 the "default"; 400 is only the small-dataset ceiling) — the same way you justify the PCA elbow choice. Convergence: if the validation ELBO rose after its best epoch (`overfitting_warning` set), prefer fewer epochs. **Early stopping is disabled under multi-GPU (DDP) training**, so a multi-GPU run *always* runs the full cap — `early_stopped=false` / "ran the full N-epoch cap" is EXPECTED there and is NOT evidence of non-convergence; judge convergence from the loss-curve plateau (and `overfitting_warning`), not from whether early stopping fired. Only on a *single-GPU* run does hitting the cap without early stopping suggest raising `max_epochs`. **The conclusive batch check in the diagnostic is the within-sample paired DEG (`cross_sample_identity_deg`).** When two clusters are each dominated by a *different* sample yet look like the same cell type, the diagnostic DEGs each one against the rest of its OWN sample and compares the two identity signatures. Because each DEG is computed entirely within a single sample, batch is held constant inside the test — so if the two within-sample signatures match (`signature_similarity` high; the pair is listed in `conclusive_pairs`), the two sample-private clusters are the same population separated only by sample: a technical batch effect that integration should merge. This is stronger than the cross-batch expression shift (which conflates biology and batch) or entropy (embedding-dependent). **When you relay a conclusive pair, explain it in full**: name the two clusters and their samples, list the shared within-sample identity genes, and give the logic — e.g. "cluster C1 (sample A) and C5 (sample B) are each the same distinct population relative to their OWN sample's background, so their separation is sample/batch-driven, not biology." A *low* similarity means the sample-private clusters are genuinely different populations and the split may be real biology — do not force integration. This evidence does not bypass the confounding caveats or the requirement that the user re-authorize integration through the runtime selector.
...
**DEGs + CellTypist + Scimilarity + the local Cytopus KnowledgeBase are the primary annotation evidence; PanglaoDB is a rare last-resort fallback.** `prepare_annotation` and the evidence validator automatically score each cluster's label against the curated, local Cytopus marker sets (relative DEG overlap) — **you do not call Cytopus yourself; it is computed for you** and surfaced as `cytopus_adjudication` / the `cytopus_plus_deg` tier. A cluster is resolved without any external query when CellTypist+Scimilarity agree + DEGs support it, OR when Cytopus's local markers best-match the cluster's DEGs for your label. Query `bc_get_panglaodb_marker_genes` **only** for the clusters left flagged `panglaodb_required` — the genuinely ambiguous cases that reference + Cytopus + DEGs cannot resolve (e.g. a label Cytopus doesn't cover such as platelets/erythroid/MAIT, reference-tool disagreement with no Cytopus match, or a cross-lineage override). Do not inflate confidence to avoid this — `panglaodb_required` is computed structurally from reference agreement, Cytopus marker overlap, ambiguity, and submitted DEG support.

**When PanglaoDB is required, use it as an adjudicator, not a rubber stamp.** The point is not to confirm what you already believe — it's to actively look for evidence that could overturn it. When you query PanglaoDB, your first question should be: "Is there a competing label that fits this cluster's DEGs better?" Check the alternatives. A cluster you labeled Neutrophil might be an inflammatory monocyte. A cluster you called ILC might be T cells. If a user gave you a marker list and it produced a label, PanglaoDB may reveal that the marker scoring was ambiguous — shared markers between lineages can systematically mislabel clusters. Surface those conflicts explicitly; don't bury them.

**Narrate your annotation evidence, every time.** For each label you assign, state which reference labels and submitted DEG markers drove the assignment, whether PanglaoDB was required, which competing labels were checked when it was required, and why the final label won. If the evidence is weak or conflicting, say so — mark the label uncertain rather than forcing a confident call. A label with no stated evidence is a label the user cannot evaluate or trust.
...
Automated labels are hypotheses, not final annotations. CellTypist and Scimilarity become strong evidence when they agree and the cluster DEGs support the same lineage; a single reference source can also be sufficient with stronger submitted DEG support. The marker-validation step is **adjudication, not confirmation**: for clusters flagged as requiring external adjudication, query PanglaoDB, compare reference markers against each cluster's DEGs, evaluate plausible competing labels, and revise unsupported labels before final reporting or saving.

### Reference-first annotation workflow

**Markers have the final say — derive from DEGs first, then reconcile (enforced).** For every cluster, the evidence must include **`deg_derived_label`**: the cell type the cluster's *own top DEGs* indicate, reasoned independently of CellTypist/Scimilarity. Reference labels are candidates, not authorities. If your `deg_derived_label` differs from the final `label`, you must also give **`deg_override_justification`** naming the specific DEGs that justify overriding the cluster's own markers — otherwise finalize is blocked. If you can't justify the override from the DEGs, use the DEG-derived label. This is how the reference-domination failure is prevented (e.g. a cluster topped by `SFTPC/SFTPB/SFTPA1` whose references say "T cell": the DEG-first read is AT2, the mismatch surfaces, and there is no honest DEG justification for the T-cell override). The harness does not judge the biology — it only enforces that you confronted the cluster's own evidence; the reasoning is yours.
...
3. **`prepare_annotation`** — pass the `cluster_key`, final `annotation_key`, optional `marker_dict`, and any reference label columns in `reference_annotation_keys` if they were not auto-detected. The tool stages per-cluster DEGs, marker scores, ambiguity flags, dominant reference candidates, `validation_tier`, `panglaodb_required_clusters`, and a bounded panel of non-nuisance DEG genes for reverse PanglaoDB lookup only where adjudication is required. **It also pre-classifies each cluster's DEGs into `discriminating_degs`, `broad_context_degs`, and `nuisance_degs`, and gives `suggested_supporting_genes`.** Use `suggested_supporting_genes` as your `supporting_genes` — they are guaranteed to pass the validator. Citing broad/MHC-II/housekeeping or nuisance genes as the supporting evidence is the #1 cause of re-staging loops; don't. The tool result returns a slim per-cluster view (the full per-cluster DEG/reference detail lives in `adata.uns['annotation_proposal']` and the full ranking in the `deg_table_csv`). **`prepare_annotation` also builds a ready-to-edit evidence scaffold in `adata.uns['annotation_evidence_scaffold']`** — one entry per cluster with `label`, `supporting_genes`, `confidence`, `reference_annotation_support`, `competing_labels_considered`, and `source_synthesis` already filled from the proposal, and `reasoning` left blank. `stage_annotation_evidence`/`finalize_annotation` overlay whatever fields you submit on top of this scaffold, so you never rebuild the evidence dict by hand — inspecting the proposal field-by-field in `run_code` to reconstruct it is the exact anti-pattern this replaces. **The proposal and scaffold persist for the whole session — do NOT re-run `prepare_annotation` just to re-read them; only re-run if the clustering actually changed (then pass `force_recompute_deg=true`).**
4. **Cytopus adjudication is automatic; PanglaoDB only for the leftovers** — `prepare_annotation` already scored each cluster's label against the local Cytopus markers (`cytopus_adjudication`, `validation_tier=cytopus_plus_deg`) and pre-filtered `panglaodb_queries_required` down to just the clusters in `panglaodb_required_clusters` that neither reference consensus nor Cytopus could resolve. Call `bc_get_panglaodb_marker_genes` **only** for those (expect this to be empty or a small handful — platelets/erythroid/MAIT, reference disagreements with no Cytopus match, cross-lineage overrides). Use the staged `panglaodb_reverse_marker_queries_required` likewise. Do not query Cytopus or PanglaoDB for clusters already in a reference/Cytopus tier. **PanglaoDB is optional, not a gate — never let it become a time sink.** Spend at most one quick PanglaoDB round on the flagged clusters. If a `panglaodb_required` cluster can't be resolved — PanglaoDB doesn't cover the label (e.g. progenitor/transitional types like CMP/GMP/MEP/early-erythroid) or the query is inconclusive — do NOT loop: stage it with `panglaodb_queried=false`, `confidence='low'`, and a one-line caveat in `reasoning` saying external adjudication was attempted but unresolved. The validator now **accepts** this and auto-caps such clusters to low confidence; it will not block finalize on a missing PanglaoDB query. DEG + CellTypist/Scimilarity + Cytopus are the primary basis — say so and move on.
5. **`stage_annotation_evidence`** — stage **all clusters in one call**. Because `prepare_annotation` already pre-filled the scaffold, **your submission only needs the fields you are adding or changing** — at minimum a `reasoning` string (≥20 chars) for every cluster. Everything else (`label`, `deg_derived_label`, `supporting_genes`, `confidence`, `competing_labels_considered`, `reference_annotation_support`, `source_synthesis`) falls back to the scaffold; you do not re-type it. Writing each cluster's `reasoning` after reviewing its DEGs/references IS the required judgment step. Override a field only when you disagree: change `label` (and `deg_derived_label`, plus a `deg_override_justification` when they differ) where your reading of the markers differs from the scaffold's proposed_label; raise/lower `confidence` where warranted. **For `needs_external_adjudication` clusters only**, after querying PanglaoDB set `panglaodb_queried=true` and a `panglaodb_label_used` you actually queried for *that* cluster; **never copy one cluster's PanglaoDB label onto another** (the validator drops biologically incompatible claims). If no PanglaoDB label genuinely matches, submit `panglaodb_queried=false` + `confidence='low'` with a one-line caveat in `reasoning` — the validator accepts reference+DEG evidence and caps to low. Reference/Cytopus-tier clusters keep the scaffold's `panglaodb_queried=false`. Pass `evidence_summary` as a **structured object** (a JSON string is also tolerated); the per-cluster payloads are tiny now (mostly reasoning), so inline is right even for many clusters. For very large cluster counts you can still `json.dump` to a file and pass `evidence_path`. **`stage_annotation_evidence` runs the same validator as `finalize_annotation`** and returns `ready_to_finalize` (bool), `clusters_failing`, `clusters_awaiting_reasoning`, and per-issue detail in `result.validation` (failing clusters only, to keep context small). Deterministic fixes (confidence caps, filled `reference_annotation_support`/`competing_labels_considered`) are applied automatically and listed in `result.validation.auto_fixes`. Resubmit only the failing clusters; prior submissions are preserved.
6. **Finalize** — call `finalize_annotation` **only when `stage_annotation_evidence` returned `ready_to_finalize: true`** (`clusters_failing` empty and every cluster has a reasoning). Do not call finalize speculatively while clusters are still failing — it runs the identical validator. Once ready, finalize commits the labels (evidence_summary can be omitted; staged evidence is reused). Because staging already validated, `finalize_annotation` should normally pass on the first call. If you call `finalize_annotation` directly without staging, the scaffold still applies: submitting only per-cluster `reasoning` is enough, but finalize refuses to write labels if `prepare_annotation` was not run, if **no** `reasoning` has been supplied for any cluster (it tells you the scaffold is filled and only reasoning is needed — this is not a persistence bug), if any cluster lacks `supporting_genes` or reasoning, or if required PanglaoDB evidence or competing-label records for ambiguous clusters are missing after auto-fill. Missing CellTypist sources must either be run or have concrete unavailable reasons recorded. Missing Scimilarity is stricter: when the package and model path are available, `finalize_annotation` requires `run_scimilarity`; a manually supplied `reference_source_unavailable` value is not enough. Nuisance genes (MT/ribosomal/hemoglobin/MALAT1/generic locus genes) cannot be the sole support. Broad-parent PanglaoDB labels, weak one-source support, and QC caveats auto-cap confidence. On success it writes `adata.obs[annotation_key]` and records full evidence in `adata.uns['annotation_validation']`. When a cluster's `supporting_genes` are rejected as not in its DEGs, the failure message now lists that cluster's pre-validated `suggested_supporting_genes` — cite those directly to fix it in one round rather than guessing. As a last resort, if finalize genuinely cannot pass after honest correction rounds, call `save_data` with `allow_unvalidated=true`: it writes a clearly-marked `_UNVALIDATED` dataset (with `adata.uns['annotation_status']='unvalidated'`) so the analysis is never lost — never abandon the run with nothing saved, and never hand-assign labels in `run_code` to dodge validation.
...
### Step 3: Conditionally adjudicate with PanglaoDB via `bc_get_panglaodb_marker_genes` (MCP tool)
For each proposed annotation label from clusters in `panglaodb_required_clusters`:
...
4. After initial scoring, compute DEGs per cluster and query PanglaoDB for top candidates and competitors only for clusters flagged as requiring external adjudication. The DEG evidence takes precedence over the marker-mean score when they conflict.
...
The user's list tells you what to look for. The DEGs tell you what's actually there; PanglaoDB adjudicates unresolved cases.
...
- Annotating clusters that require external adjudication **without PanglaoDB validation** — always call `bc_get_panglaodb_marker_genes` for `panglaodb_required_clusters`, but do not flood PanglaoDB for reference+DEG-sufficient clusters.
