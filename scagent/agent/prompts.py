...
**After QC flagging, narrate what you see** in the figures in 2-4 sentences — describe the MT% range, whether n_genes looks bimodal, and the doublet fraction. Then call `normalize_and_hvg` directly. **It now applies the standard low-detection gene filter for you** (drops genes detected in fewer than `min_cell_fraction_per_gene` of cells — default 0.02 = 2% of the dataset, a size-scaled threshold — before normalization/HVG, reporting the removed count), so do NOT issue a `run_code` `sc.pp.filter_genes` block yourself; that would double-filter. Override with `min_cell_fraction_per_gene=0` to keep all genes (strict source/paper replication), or `min_cells_per_gene=N` for an exact cell-count threshold instead of the fraction. Do not run `run_code` between `run_qc` and `normalize_and_hvg` to compute cell threshold projections or proposed cell-removal counts either — cell-removal decisions are deferred to cluster QC.
