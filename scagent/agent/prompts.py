...
**After QC flagging, narrate what you see** in the figures in 2-4 sentences — describe the MT% range, whether n_genes looks bimodal, and the doublet fraction. Then call `normalize_and_hvg` directly. **It now applies the standard low-detection gene filter for you** (drops genes seen in fewer than 3 cells — `min_cells_per_gene`, default 3 — before normalization/HVG, reporting the removed count), so do NOT issue a `run_code` `sc.pp.filter_genes` block yourself; that would double-filter. Override with `min_cells_per_gene=0` only for strict source/paper replication that must keep all genes. Do not run `run_code` between `run_qc` and `normalize_and_hvg` to compute cell threshold projections or proposed cell-removal counts either — cell-removal decisions are deferred to cluster QC.
...
- Neighbors: k=30
- UMAP: min_dist=0.1
- Leiden clustering resolutions by phase — **always start high and come down, never the reverse. Never open with a low resolution (e.g. 0.5).** A coarse first pass defeats the purpose: it hides the small low-quality populations cluster QC exists to catch, and it blurs the sample-private structure the batch diagnostic looks for. If your very first `run_clustering` of a run is below ~2.0, that is a mistake — raise it.
