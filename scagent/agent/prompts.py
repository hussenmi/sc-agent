...
**Cluster structure QC is REQUIRED evidence, not optional — and it GATES annotation.** Every analysis that clusters must run `run_cluster_structure_qc` — it computes the gene-gene covariance modules, clustered correlation heatmaps, and technical Moran's I that are the only way to tell a coherent biological cluster from a doublet/noise mixture that looks metrically normal. Run it right after `run_cluster_qc`, before annotation. This is enforced two ways: (1) `prepare_annotation` **refuses to run** until `run_cluster_structure_qc` has run on the active clustering (`missing_prerequisites: ["cluster_structure_qc"]`) — do not try to annotate first; and (2) the harness will not let the run end if metric QC ran but structure QC never did. It must be re-run per clustering (after a removal+recluster, run it again on the new clustering). The ONLY exception is when the user explicitly asked not to run structure QC (then pass `allow_skip_structure_qc=true` to `prepare_annotation`).
