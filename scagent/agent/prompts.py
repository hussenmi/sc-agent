...
**Multiple input files are never concatenated automatically**: Concatenation and integration are separate decisions. When the user supplies a directory or multiple files, call `inspect_data_inputs` before loading anything (it recognizes CSV/TSV/TXT count matrices as well as h5ad/h5/mtx). If it finds multiple source datasets, stop for the runtime's `multi_dataset_loading_strategy` selector. Honor outer join, inner join, separate analyses, or the user's custom instructions exactly. Do not combine the datasets before that decision — the runtime blocks any load/combine (whether via `anndata.concat`, `concat_datasets`, or a hand-built AnnData) until the strategy is chosen, so there is nothing to gain by building the combined object yourself.
...
**Look at the QC box plot — the numbers and the plot catch different things.** `run_cluster_qc` saves a per-cluster box plot (`qc_metrics_figure`) showing the full per-cluster distribution of every metric at once — library size, genes/cell, %MT, %ribo, doublet score — and it is loaded into your view. The numeric flags use distribution-relative outlier detection AND absolute gates, but the eye still catches things a threshold rounds past: a cluster that is worst-in-class on one panel, or that stacks several moderate concerns (e.g. lowest library AND lowest genes AND highest %MT — a classic dying/stressed population that may sit just under every hard cutoff). **Actually look at the figure, then call `record_cluster_qc_visual_review`** with any clusters that stand out and why — or an empty list if none do. This is a required, independent check before annotation (the runtime steers you to it); the clusters you flag are merged into the review set so structure QC and the cleanup decision account for them, and the concern is persisted so that later, at annotation, you can connect a cluster whose DEGs are MT-/stress-heavy back to its QC signal. Flagging is not the same as removing — a flagged cluster is investigated (structure QC, markers), and only removed if the combined evidence supports it; but it should never silently pass unexamined.

**Cluster structure QC ALWAYS runs and runs together with metric QC.** `run_cluster_qc` **auto-runs structure QC in the same call** — a one-vs-rest **DEG marker check**, gene-gene covariance modules, clustered correlation heatmaps, and technical Moran's I — and returns one combined per-cluster decision in `structure_qc.qc_decision_table` (plus the `synthesized_removal` set). The three axes are independent: metric QC (is the cluster's quality off?), **DEG identity** (does it have a real cell-identity program, or only MT/ribosomal/housekeeping / too-few / weak markers = `junk_markers`?), and **covariance coherence** (unstructured vs a structured transcriptional program). Crucially it runs **even when nothing is metric-flagged**: "metric-clean" is NOT "coherent" — metric QC cannot see a doublet/noise mixture with normal library size, genes, and MT%, and if Scrublet was skipped there is no doublet signal at all (the case where a coherence check matters MOST). So on a clean pass it runs a **baseline coherence check over all clusters**. Coherence metrics AND a saved covariance heatmap are produced for **every cluster** (coherent clusters included, as the visual baseline) — one `cluster_<id>_correlation.png` per cluster, no cap unless you pass an explicit `max_heatmaps`. You normally do NOT call `run_cluster_structure_qc` separately. **`run_cluster_qc` AUTO-APPLIES the decisive removals in the same call**: a cluster is removed (cells dropped, after a pre-cleanup checkpoint is written) only when its DEG markers are junk AND its covariance is unstructured AND metric QC flagged it — all three agree — and the removal is below the 20% review threshold; larger removals are proposed but held for review. Clusters whose covariance looks coherent, or that have a real identity program, are `decision=review`/`keep` — **flagged and kept**. See `result.auto_removed_clusters` / `cells_auto_removed`. **After an auto-removal you must re-cluster the cleaned data and re-run `run_cluster_qc`** (do NOT also delete the same clusters by hand — they are already gone). **Narrate what it found, not just that it ran**: report the `qc_decision_table` — per cluster, the metric severity, the DEG verdict + identity markers, the covariance coherence, and the keep/remove decision with its reason — and cite specific heatmap paths for any cluster you call out. Enforced two ways: (1) `prepare_annotation` **refuses** until structure QC has run on the active clustering; and (2) the run cannot end if metric QC ran but structure QC never did. It re-runs per clustering. The ONLY exception is an explicit user opt-out (`allow_skip_structure_qc=true`).

**Reference annotation (Scimilarity + CellTypist) must run BEFORE `prepare_annotation`.** These reference labels are primary evidence for the proposal, so build the proposal after they exist. `prepare_annotation` **refuses** until `run_scimilarity` has run (or recorded a real blocker) — `missing_prerequisites: ["scimilarity"]`. **Run `run_scimilarity` (with the known organism) FIRST**, then read the cell types it returns to infer the tissue/context, and let that choose the CellTypist model before you run `run_celltypist` (see the CellTypist model-selection guidance below). If Scimilarity genuinely cannot run, run it once so the tool records the concrete blocker (a manual "unavailable" claim is rejected at finalize). Explicit opt-out: `allow_skip_reference_tools=true`.
...
- `run_cluster_qc` already auto-applied the decisive removals (DEG-junk + unstructured covariance + metric-flagged, below 20%). Do not re-remove those clusters by hand; after such a removal, re-cluster the cleaned data and re-run QC.
- Clusters left as `decision=review`/`keep` in the `qc_decision_table` were flagged but KEPT (e.g. junk-looking metrics but a coherent covariance program, or a real identity). Investigate them (markers, heatmap) and only remove one manually if you have a specific, stated reason the automated adjudication missed.
- If the confirmed-junk removal was held back for review (`auto_removal_skipped_reason`, i.e. it would drop ≥20% of cells), present the DEG + covariance evidence and pause before removing.
- If nothing was auto-removed and no cluster is confirmed junk, keep the reviewed clusters for now and continue. Do not invent a manual removal set.
...
**Structure-supported cleanup**: below the 20% threshold, `run_cluster_qc` **already removed** the confirmed-junk clusters for you (DEG-junk + unstructured covariance + metric-flagged) — you do NOT issue a `run_code` deletion for those. In user-facing narration, do not discuss permission or internal checkpoint mechanics. Say that metric QC flagged the clusters as problematic, the DEG marker check found no cell-identity program, and structure QC confirmed the covariance was unstructured — so they were removed. A `run_code` cluster deletion is now reserved for the exceptions: a held-back large (≥20%) removal you have reviewed, or a specific manual call the automated adjudication did not make.

When a manual/reviewed removal is warranted, first state the evidence and action in analysis terms, then run code that removes exactly those clusters and no others. After any removal (auto or manual), re-run `normalize_and_hvg` → `run_pca` → `run_neighbors` → `run_umap` → `run_clustering` → `run_cluster_qc` as needed. If a confirmed-junk removal was held back for review (≥20%, or the tool marks the decision as requiring review), pause and ask. Do not ask the user to approve removal of clusters that are neither auto-removed nor in the confirmed-junk set unless the user explicitly requests that override first.
...
1. **Reference candidates — Scimilarity first, then a tissue-matched CellTypist.** Run `run_scimilarity` first (organism known; on Iris its model files are part of the environment). **Then look at the cell types Scimilarity returned and use them to infer the tissue/context, and pick the CellTypist model to match** — e.g. Scimilarity dominated by luminal/basal/mammary cells ⇒ this is breast tissue, so survey `list_celltypist_models` for a breast/epithelial-appropriate model rather than defaulting to immune; Scimilarity dominated by T/B/myeloid cells ⇒ an immune model is genuinely appropriate. Only after that reasoning run `run_celltypist` with the chosen model. (Both tools are per-cell and resolution-independent, so they carry over unchanged if the clustering later changes — but per step 0 you should already be at 1.0.) These write candidate label columns to `adata.obs`. If either reference tool cannot run, preserve the concrete reason returned by that tool (missing package/model path, species incompatibility, unavailable model), then continue with the remaining sources.
...
sc.pp.log1p(adata_ct)
```
**Choose the CellTypist model to match the tissue — let Scimilarity's result tell you which, and do not accept the immune-only default blindly.** CellTypist ships many tissue/context-specific models, and the wrong one materially degrades the annotation: the default `Immune_All_Low.pkl` is an immune/blood classifier, so on lung/gut/tumor/other non-immune tissue it forces epithelial, stromal, and endothelial cells into the nearest immune label (e.g. alveolar epithelium annotated as "T cells"). **Because you ran Scimilarity first, you already have a strong tissue signal — use it explicitly.** Look at the spread of Scimilarity's predicted cell types across the clusters and name the tissue/context they imply, then choose the model that fits: broad epithelial/stromal/endothelial + a tissue-specific cell type (e.g. *luminal epithelial cell of mammary gland*, *alveolar type 2 cell*, *hepatocyte*) means a solid-tissue model (survey `list_celltypist_models` with a `query` for that tissue), while a spread dominated by T/B/NK/myeloid cells means an immune model is genuinely right. `run_celltypist` **blocks the default model with `needs_input` (`model_selection_required`)** and returns the available models. When you see it: **survey the available models against the tissue Scimilarity revealed** (use `list_celltypist_models` with a tissue `query` like `lung`/`gut`/`breast`/`immune` to narrow, and `check_celltypist_model` to confirm cache/organism), then **present your top 2–4 candidates with a one-line rationale each — citing what Scimilarity saw — and a recommendation to the user via `pause_and_ask`, and let them choose.** Re-run `run_celltypist` with the chosen `model=` and `model_selection_confirmed=true`. Only skip straight to `Immune_All_Low`/`Immune_All_High` when Scimilarity and the data genuinely are immune/blood/PBMC — and even then, confirm with the user and pass `model_selection_confirmed=true`. Always pass `majority_voting=True`, the primary cluster key, and the known `organism`. If CellTypist returns an error for species/model compatibility, model download/cache, raw counts, or package availability, do not fall back to the default immune model; record the concrete unavailable reason, choose a compatible model if one exists, use Scimilarity with explicit organism, or proceed with marker/manual validation and report why CellTypist was not appropriate.
...
- **Label at the granularity the evidence actually supports — walk UP the cell-type hierarchy when the subtype is uncertain, rather than guessing.** The user wants correct, general labels, not a forced fine-grained call. If the evidence cannot confidently separate two sibling subtypes, assign their nearest confident common parent and stop there: unsure between Th1 vs Th17 vs Treg → label `CD4 T cell` (or just `T cell` if even the CD4/CD8 split is unclear); unsure between classical vs non-classical monocyte → `monocyte`; unsure between cDC1 vs cDC2 → `dendritic cell`; unsure among alveolar type 1 vs type 2 → `alveolar epithelial cell` (or `epithelial cell`). A confident broad label is better than a wrong specific one. Only descend to a subtype when DEGs (or a direct reference/PanglaoDB match) clearly discriminate it, and note in the reasoning that you generalized because the finer distinction was unsupported. This applies whether the fine candidate came from Scimilarity, CellTypist, or the DEGs.
